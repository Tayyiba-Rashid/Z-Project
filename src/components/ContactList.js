...
function ContactList({ selectUser }) {
  ...
  useEffect(() => {
    ...
    const fetchUsers = async () => {
      ...
      try {
        ...
        const userList = querySnapshot.docs
          ...
          .filter((u) => u.email !== currentUser.email); // Filter out current user
    ...
    fetchUsers();
  }, [currentUser]); // Re-fetch users if currentUser changes

  const handleSelectUser = (user) => {
    setSelectedUserId(user.id); // Update the selected user ID
    selectUser(user); // Call the parent handler
  };
        ...
        users.map((user) => (
          <div
            ...
            className={`cursor-pointer p-3  bg-white border-b rounded-sm  ${
              selectedUserId === user.id ? "bg-gradient-to-r from-blue-500 to-purple-400 text-white" : "hover:bg-gray-50"
            }`}
          >
